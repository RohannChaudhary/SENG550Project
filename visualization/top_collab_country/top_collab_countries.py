...
# Plot the network
plt.figure(figsize=(15, 10))  # Increase canvas size for better spacing

# Position nodes using the spring layout with adjustments
pos = nx.spring_layout(G, seed=42, k=0.5, iterations=50)  # Increase k for better spacing

# Draw nodes and labels
nx.draw_networkx_nodes(G, pos, node_size=700, node_color="#E50914", alpha=0.9)  # Netflix red for nodes
...
edges = nx.draw_networkx_edges(
    ...
    width=[G[u][v]['weight'] / 10 for u, v in G.edges()],  # Edge width proportional to shared_titles_count
    alpha=0.6  # Add transparency to reduce clutter
)
